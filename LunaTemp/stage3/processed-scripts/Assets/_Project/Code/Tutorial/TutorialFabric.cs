...
namespace _Project.Code.Tutorial
{
    ...
    public class TutorialFabric : MonoBehaviour
    {
        ...
        [SerializeField] private Transform _joystickHandle;
        [SerializeField] private GameObject _joystickGuide;

        [SerializeField] private Transform _storageSpawnPoint;
        [SerializeField] private Buyable _tableBuyable;
        ...
        {
            IStorage Create() =>
                _storageFabric.CreateNew(_storageSpawnPoint.position, Core.Items.ItemType.Coin, 10);

            return new Tutorial(
                _coroutineRunner, Create, _joystick,
                new TweenEnable(AnimationsFactory.CreateInfinitySign(_joystickHandle, 1.5f)),
                _joystickGuide,
                new TutorialArrow(_arrowHolder, _arrow, _point1, _point2),
                _tableBuyable);
        }
    }
}