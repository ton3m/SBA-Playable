...
namespace _Project.Code.Core.Workers
{
	public class FieldAnimation : MonoBehaviour
	{
		...
		[FormerlySerializedAs("_workerOffset")]
		[SerializeField]
		...
		{
			_worker.DOScale(Vector3.one * 1.1f, 0.3f).SetLoops(-1, LoopType.Yoyo).Play()
				.Goto(Random.Range(0f, 1f));
			yield return null;
		}

		private IEnumerator AnimateWorker()
		{
			_ = _plants[0].position;
			Transform[] plants = _plants;
			Vector3 endPoint = plants[plants.Length - 1].position + _endPointOffset;
