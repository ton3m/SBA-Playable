...
namespace _Project.Code.Core.Items.Handlers
{
	public class ItemCollisionHandler : IDisposable
	{
		...
		{
			if (type != CollisionType.Exit && obj.TryGetComponent<Item>(out var item) && item.enabled && _provider.TryGetStorage(item.Type, out var storage))
			{
				bool result = _transferer.TryTransfer(item, storage);
